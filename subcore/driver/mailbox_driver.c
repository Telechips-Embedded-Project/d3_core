...
/* userspace writes a whole struct tcc_mbox_data */static ssize_t mbox_test_write(struct file *filp, const char __user *buf,
                              ...
                              size_t len, loff_t *ppos)
{
    ...
    /* 먼저 peeking으로 data_len 파악 (FIFO에서 빼지 않음) */
    mutex_lock(&mdev->rx_lock);
    got = kfifo_out_peek(&mdev->rx_fifo, &kmsg, sizeof(kmsg));
    if (got != sizeof(kmsg)) {
        mutex_unlock(&mdev->rx_lock);
        return -EAGAIN; /* 드물지만 race 방지 */
    }

    /* 이 메시지를 유저 버퍼에 얼마만큼 줄지 계산 */
    if (kmsg.data_len > MBOX_DATA_FIFO_SIZE) {
        mutex_unlock(&mdev->rx_lock);
        return -EIO; /* 손상된 메시지 방어 */
    }
    msg_size = hdr_sz + ((size_t)kmsg.data_len * 4);

    if (len < msg_size) {
        mutex_unlock(&mdev->rx_lock);
        return -EMSGSIZE; /* 유저 버퍼가 부족하면 메시지는 그대로 보존 */
    }
...
static __poll_t mbox_test_poll(struct file *filp, poll_table *wait)
{
	...
	if (kfifo_len(&mdev->rx_fifo) >= sizeof(struct tcc_mbox_data))
		mask |= POLLIN;
	/* writes are generally allowed; errors reported on write() */
	mask |= POLLOUT;
...
static int mbox_test_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "ready: /dev/%s (channel \"%s\")\n",
...
MODULE_AUTHOR("Telechips / (modified for RW) ");
MODULE_DESCRIPTION("Telechips Multi-channel Mailbox RW client");
MODULE_LICENSE("GPL");
